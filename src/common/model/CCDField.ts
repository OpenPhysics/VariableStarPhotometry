...
function besselJ1( x: number ): number {
  ...
  if ( ax < 8.0 ) {
    ...
    const a1 = 1.0 + y * ( 0.183105e-2 + y * ( -0.3516396496e-4
      + y * ( 0.2457520174e-5 + y * ( -0.240337019e-6 ) ) ) );
    const a2 = 0.04687499995 + y * ( -0.2002690873e-3
...
function buildNoiseData( numChunks: number, chunkSize: number ): Float64Array {
  ...
  for ( let i = 0; i < n; i++ ) {
    ...
    do {
      x1   = 2 * ( seed / 2147483647 ) - 1; seed = pmNext( seed );
      x2   = 2 * ( seed / 2147483647 ) - 1; seed = pmNext( seed );
      fv   = x1 * x1 + x2 * x2;
    } while ( fv >= 1 );
...
function buildGammaLUT( invert: boolean ): Uint8Array {
  ...
  for ( let v = 0; v <= peakValue; v++ ) {
    let g = Math.round( 255 * Math.pow( v / peakValue, inv ) );
    if ( g < 0 )   g = 0;
    if ( g > 255 ) g = 255;
...
export class CCDField {
  ...
  private buildChunkTable( noiseSeed: number ): Int32Array {
    ...
    for ( let i = 0; i < this.numChunks - 1; i++ ) {
      ...
      seed = pmNext( seed );
      const tmp = tbl[ j ] as number; tbl[ j ] = tbl[ i ] as number; tbl[ i ] = tmp;
    }
    return tbl;
  }
  ...
  private starMagnitude( starIndex: number, epoch: number ): number {
    ...
    switch ( s.type ) {
      case 'constant':  return s.magnitude;
      case 'pulsating': return getPulsatingMagnitude( s.prototypeName, s.centerMagnitude, epoch );
      case 'eclipsing': return getEclipsingMagnitude( s.prototypeName, s.peakMagnitude, epoch );
    }
  }
  ...
  private buildFieldData( obsIndex: number ): { fieldData: Float64Array; chunkTable: Int32Array } {
    ...
    for ( let si = 0; si < STARS.length; si++ ) {
      ...
      const f    = peakValue * Math.pow( 10, ( saturationMagnitude - mag ) / 2.5 );
  ...
  // -------------------------------------------------------------------------
  public getApertureStats(
    obsIndex: number,
    cx: number,
    cy: number,
    radius: number
  ): { totalCounts: number; totalPixels: number; average: number } {
    const { fieldData, chunkTable } = this.buildFieldData( obsIndex );
  ...
  public get width(): number  { return W; }
  /** Field height in pixels (constant). */
  public get height(): number { return H; }
}
